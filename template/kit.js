...
// Reveal by user-perceived character, not by code point. [...s] splits "กำ"
// into a consonant and a vowel mark, and revealing the first of those puts a
// mark on screen with nothing under it for a frame. segmentGraphemes is
// declared below; typed is only ever called at render time, by which point it
// exists.
const typed = (s, p) => { const a = segmentGraphemes(s); return a.slice(0, Math.round(a.length * clamp(p))).join(''); };
...
// The locale every text routine segments with. Thai is the point of this fork;
// set it to another tag and the same code keeps working, because Intl.Segmenter
// is doing the language-specific part rather than this file.
const LANG = 'th';
...
// Split a line into the units kinetic type pops in one at a time.
//
// The original did s.split(' '), which is correct for languages that put
// spaces between words and useless for Thai, which does not. A Thai sentence
// came back as one element and the whole line arrived as a single blob — the
// signature effect of this engine simply did not work in Thai.
//
// Intl.Segmenter carries ICU's Thai dictionary and is already in the renderer,
// because the renderer is headless Chrome. No dictionary ships with this skill.
// Latin still falls out of the same call correctly, so there is one code path
// rather than a language flag somebody has to remember to set.
function segmentWords(s) {
  if (typeof Intl === 'undefined' || !Intl.Segmenter) return s.split(' ');
  const seg = new Intl.Segmenter(LANG, { granularity: 'word' });
  const out = [];
  for (const part of seg.segment(s)) {
    // Keep punctuation and spacing attached to the word before it rather than
    // letting a comma pop in on its own like a word.
    if (part.isWordLike || out.length === 0) out.push(part.segment);
    else out[out.length - 1] += part.segment;
  }
  return out.map(w => w.trim()).filter(Boolean);
}

// Split into user-perceived characters for the typewriter.
//
// "กำ" is two code units and one syllable. Revealing it with .slice() puts a
// bare vowel mark on screen for a frame, floating with nothing to sit on.
function segmentGraphemes(s) {
  ...
  return [...new Intl.Segmenter(LANG, { granularity: 'grapheme' })
    .segment(s)].map(part => part.segment);
}
...
function kwords(s, x, y, p, o = {}) {
  ...
  // Segmentation decides the order things arrive in. It must not decide the
  // spacing, and in Thai those are different questions.
  //
  // Rendered and looked at: laying the segments out with a uniform gap turned
  // 'โรงงานน้ำแข็งสยามอาร์กติก' into 'โรงงาน น้ำ แข็ง สยาม อาร์กติก' on screen. Thai
  // words run together; a gap between them is not emphasis, it is a spelling
  // mistake — and ICU had split a proper noun it does not know, which only
  // became visible because the layout made the split visible.
  //
  // So space appears where the writer put a space, and nowhere else. Latin is
  // unaffected: its segments are separated by real spaces to begin with.
  const words = segmentWords(s), ws = words.map(w => measure(w, font, size, weight)), sp = size * gap;
  const gaps = words.map((w, i) => (i === words.length - 1 ? 0 : (s.includes(w + ' ') ? sp : 0)));
...
// ---------------------------------------------------------------- faces
// สีหน้าวาดทับสไปรต์ 16 แถวแบบ PERSON (ตาอยู่แถว index 4 นับจากบน)
// ใช้กับ mini ที่วาดด้วย drawMini(key, x, y, s) — x,y คือจุดเท้า, s คือขนาดพิกเซล
//
// ทำไมต้องมี: ตัวละครที่หน้าเดียวทั้งเรื่องทำให้คนดูไม่รู้สึกไปด้วย
// ตอนของหาย ตอนถูกถาม ตอนดีใจ ควรหน้าไม่เหมือนกัน
//
// mood: 'worried' คิ้วปลายในยกขึ้น ปากเบ้  (ระวัง: ปลายในตกลง = โกรธ ไม่ใช่กังวล)
//       'shocked' ตาโตกลม ปากอ้า
//       'tired'   เปลือกตาครึ่ง
//       'relieved' ตาหยีโค้ง ยิ้ม
function face(x, y, s, mood, { flip = false, bob = 0, skin = '#dfb087', ink = THEME.ink } = {}) {
  if (!mood || mood === 'neutral') return;
  const ey = y - bob - s * 11.5, cx = x;
  const ln = (pts, w) => { ctx.beginPath(); pts.forEach(([a, b], i) => i ? ctx.lineTo(a, b) : ctx.moveTo(a, b)); ctx.strokeStyle = ink; ctx.lineWidth = w; ctx.lineCap = 'round'; ctx.stroke(); };
  const dot = (a, b, r) => { ctx.beginPath(); ctx.arc(a, b, r, 0, TAU); ctx.fillStyle = ink; ctx.fill(); };
  const arc = (a, b, r, a0, a1, w) => { ctx.beginPath(); ctx.arc(a, b, r, a0, a1); ctx.strokeStyle = ink; ctx.lineWidth = w; ctx.stroke(); };
  const wipe = (h0, h1) => { ctx.fillStyle = skin; ctx.fillRect(cx - s * 2.2, ey - s * h0, s * 4.4, s * h1); };
  ctx.save();
  if (mood === 'worried') {
    ln([[cx - s * 2.4, ey - s * 1.1], [cx - s * .9, ey - s * 2.0]], s * .45); ln([[cx + s * 2.4, ey - s * 1.1], [cx + s * .9, ey - s * 2.0]], s * .45);
    arc(cx, ey + s * 3.4, s * 1.1, Math.PI * 1.15, Math.PI * 1.85, s * .45);
  } else if (mood === 'shocked') {
    wipe(.7, 1.6); dot(cx - s * 1.3, ey, s * .75); dot(cx + s * 1.3, ey, s * .75); dot(cx, ey + s * 2.6, s * .7);
  } else if (mood === 'tired') {
    wipe(.7, .9); ln([[cx - s * 1.9, ey - s * .1], [cx - s * .7, ey - s * .1]], s * .5); ln([[cx + s * .7, ey - s * .1], [cx + s * 1.9, ey - s * .1]], s * .5);
  } else if (mood === 'relieved') {
    wipe(.8, 1.7);
    arc(cx - s * 1.3, ey + s * .2, s * .7, Math.PI * 1.1, Math.PI * 1.9, s * .5); arc(cx + s * 1.3, ey + s * .2, s * .7, Math.PI * 1.1, Math.PI * 1.9, s * .5);
    arc(cx, ey + s * 2.2, s * 1.2, Math.PI * .15, Math.PI * .85, s * .5);
  }
  ctx.restore();
}
// เหงื่อหยดข้างขมับ ใช้คู่ worried/shocked
function sweat(x, y, s, t, { flip = false, color = '#075985' } = {}) {
  const d = flip ? -1 : 1, ph = (t * 1.6) % 1, sx = x + d * s * 4.2, sy = y - s * 13 + ph * s * 3;
  ctx.save(); ctx.globalAlpha *= 1 - ph; ctx.fillStyle = color;
  ctx.beginPath(); ctx.moveTo(sx, sy - s * 1.1); ctx.quadraticCurveTo(sx + s * .9, sy + s * .6, sx, sy + s * .9); ctx.quadraticCurveTo(sx - s * .9, sy + s * .6, sx, sy - s * 1.1); ctx.fill(); ctx.restore();
}
