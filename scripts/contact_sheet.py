# Tile stills/t<sec>.png into review sheets of 6 (2x3; portrait stills 3x2), labelled with their time.
...
for n in range(0, len(files), 6):
    ...
    cols = 3 if first.height > first.width else 2
    rows = 3 if cols == 2 else -(-len(files[n:n + 6]) // 3)
    tw = 1600 // cols
